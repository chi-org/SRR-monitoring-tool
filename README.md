...
**Network monitoring dashboard consuming shared resources from SRR-shared-resources.**

This repository receives automatic updates of Grafana dashboards, query rules, and table view templates from the central shared resources repository.

---

## What This Repo Does

Network monitoring and visualization tool that uses shared resources for:
...
├── resource/                                    # Synced folders
│   ├── grafana-dashboard/                       # Grafana dashboards (synced)
│   ├── query-rule/                              # Query rules (synced)
│   └── tableview/                               # Table view templates (synced)
└── [other monitoring-tool specific files]
```
...
1. Shared resources repo pushes update to `grafana-dashboard/`, `query-rule/`, or `tableview/`
...
1. Edit files in `resource/grafana-dashboard/`, `query-rule/`, or `tableview/`
...
### Editing Synced Resources

```bash
# 1. Edit resource
vim resource/grafana-dashboard/network-overview.json

# 2. Commit and push to main
git add resource/grafana-dashboard/network-overview.json
git commit -m "Update dashboard for new metrics"
...
**Trigger:** Push to `main` affecting `resource/grafana-dashboard/`, `query-rule/`, or `tableview/`
...
**Check:**
1. Files changed match expected folders (grafana-dashboard, query-rule, tableview)
...
**Cause:** Changed files outside `resource/grafana-dashboard/`, `query-rule/`, or `tableview/`
...
```
[auto-sync][monitoring-tool] update shared folders
...
Example: Shared repo deletes `grafana-dashboard/old-dashboard.json` → deleted here on next sync.
...
1. Update `.github/consumer-config.yml` in shared repo:
   ...
   monitoring-tool:
     folders:
       ...
       query-rule: resource/query-rule
       tableview: resource/tableview
       new-folder: resource/new-folder  # Add this
   ```
